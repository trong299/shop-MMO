...
import { baseStats, enemyKinds, getStageConfig, itemNames, rarityColors, rarityOrder, slotIcons, slots, TILE_SIZE, upgrades, } from "./data";
...
export class Game {
    ...
    start(floor = this.save.selectedFloor) {
        this.audio.unlock();
        this.stage = getStageConfig(Math.max(1, Math.min(this.save.unlockedFloor, floor)));
        this.save.selectedFloor = this.stage.floor;
        this.persistSave();
        this.maze = generateMaze(this.stage.mazeWidth, this.stage.mazeHeight, this.stage.treasureCount, this.stage.trapCount, Date.now());
    ...
    update(dt) {
        ...
        if (!this.bossSpawned) {
            ...
            if (this.spawnTimer <= 0) {
                ...
                if (living < this.stage.enemyCount + Math.floor(this.elapsed / 25))
                    this.spawnEnemy();
    ...
    updateInput(dt) {
        ...
        if (gamepad) {
            const deadzone = 0.18;
            if (Math.abs(gamepad.axes[0] ?? 0) > deadzone)
                dx = gamepad.axes[0];
            if (Math.abs(gamepad.axes[1] ?? 0) > deadzone)
                dy = gamepad.axes[1];
            ...
            if (gamepad.buttons[0]?.pressed && this.nearestChest)
                this.openChest(this.nearestChest);
            if (gamepad.buttons[1]?.pressed && this.player.dashReady <= 0)
                this.dash(dx, dy);
        }
        ...
        if ((this.mouse.down || this.gamepadAttack || keyMap.has("KeyJ")) && this.player.shootCooldown <= 0)
            this.shoot(angle);
    }
    movePlayer(dx, dy) {
        const nextX = this.player.x + dx;
        if (this.circleWalkable(nextX, this.player.y, this.player.radius))
            this.player.x = nextX;
        const nextY = this.player.y + dy;
        if (this.circleWalkable(this.player.x, nextY, this.player.radius))
            this.player.y = nextY;
    }
    dash(dx, dy) {
        if (this.player.dashReady > 0)
            return;
        ...
        for (let i = 0; i < 14; i += 1)
            this.addParticle(this.player.x, this.player.y, "#65f1d0", 3, 120);
    }
    shoot(angle) {
    ...
    updateEnemies(dt) {
        for (const enemy of this.enemies) {
            ...
            if (enemy.boss) {
                ...
                if (enemy.specialCooldown <= 0) {
                    if (enemy.phase === 1)
                        this.radialBurst(enemy, 8, "#e75d88");
            ...
            if (distance < this.player.radius + enemy.radius + 3 && enemy.attackCooldown <= 0) {
                ...
                if (this.player.thorns > 0)
                    this.damageEnemy(enemy, enemy.damage * this.player.thorns, false);
            }
        }
        if (this.player.auraDamage > 0 && this.player.auraTimer <= 0) {
            ...
            for (let i = 0; i < 12; i += 1)
                this.addParticle(this.player.x, this.player.y, "#ff7a3d", 3, 90);
        }
    }
    enemyDirection(enemy, directX, directY) {
    ...
    moveEnemy(enemy, dx, dy) {
        ...
        if (this.circleWalkable(nextX, enemy.y, enemy.radius))
            enemy.x = nextX;
        const nextY = enemy.y + dy;
        if (this.circleWalkable(enemy.x, nextY, enemy.radius))
            enemy.y = nextY;
    }
    enemyShot(enemy, angle) {
        this.activateProjectile({
            x: enemy.x,
            y: enemy.y,
    ...
    updateProjectiles(dt) {
        for (const projectile of this.projectiles) {
            ...
            for (const enemy of this.enemies) {
                ...
                if (this.player.chainChance > 0 && this.random.next() < this.player.chainChance)
                    this.chainLightning(enemy, damage * 0.55);
                projectile.pierce -= 1;
    ...
    damageEnemy(enemy, amount, critical, color) {
        if (enemy.dead)
            return;
        enemy.hp -= amount;
        enemy.hitFlash = 0.09;
        ...
        for (let i = 0; i < (critical ? 8 : 3); i += 1)
            this.addParticle(enemy.x, enemy.y, color ?? enemy.kind.accent, critical ? 4 : 2, 110);
        if (critical)
    ...
    killEnemy(enemy) {
        ...
        this.save.gold += gold;
        if (enemy.elite)
            this.ui.toast(`Elite defeated · +${gold} gold`, "#ffbf58");
    }
    chainLightning(source, amount) {
    ...
    damagePlayer(rawDamage) {
        if (this.player.invulnerable > 0 || this.gameOver)
            return;
    ...
    openChest(chest) {
        if (chest.opened || this.paused)
            return;
        chest.opened = true;
        this.paused = true;
        this.audio.chest();
        for (let i = 0; i < 28; i += 1)
            this.addParticle(chest.x, chest.y, chest.secret ? "#d278ff" : "#ffd36b", 4, 190);
        const options = [];
        const upgradePool = this.random.shuffle([...upgrades]).filter((upgrade) => (this.upgradeStacks.get(upgrade.id) ?? 0) < upgrade.maxStacks);
    ...
    generateEquipment(rarityBonus = 0) {
        ...
        const stats = {};
        if (slot === "Weapon")
            stats.attack = Math.round(7 * multiplier);
        if (slot === "Helmet") {
            stats.maxHp = Math.round(10 * multiplier);
            stats.critChance = 0.01 * (rarityIndex + 1);
        }
        if (slot === "Armor") {
            stats.defense = Math.round(3 * multiplier);
            stats.maxHp = Math.round(8 * multiplier);
        }
        if (slot === "Boots")
            stats.moveSpeed = Math.round(8 * multiplier);
        if (slot === "Ring") {
            stats.critChance = 0.018 * (rarityIndex + 1);
            stats.critDamage = 0.08 * (rarityIndex + 1);
        }
        if (slot === "Amulet") {
            stats.lifeSteal = 0.008 * (rarityIndex + 1);
            stats.attack = Math.round(2 * multiplier);
        }
        if (slot === "Artifact") {
            stats.elementDamage = Math.round(3 * multiplier);
            stats.cooldownReduction = 0.015 * (rarityIndex + 1);
        }
    ...
    updateTraps() {
        ...
        if (!current || current.room !== "trap")
            return;
        const key = `${cell.x}:${cell.y}`;
        if (this.visitedTraps.has(key))
            return;
        ...
        for (let i = 0; i < 18; i += 1)
            this.addParticle(this.player.x, this.player.y, "#ff633f", 4, 160);
    }
    updateParticles(dt) {
        ...
        while (this.particles.length > 650)
            this.particles.shift();
        while (this.texts.length > 80)
            this.texts.shift();
    }
    completeStage() {
        ...
        window.setTimeout(() => {
            this.ui.showStageComplete(this.stage.floor === 20 ? "THE ETERNAL MAZE CONQUERED" : `FLOOR ${String(this.stage.floor).padStart(2, "0")} CLEARED`, rewards);
        }, 800);
    }
    endRun() {
        this.gameOver = true;
        this.paused = true;
        this.persistSave();
        this.ui.showGameOver(`Reached Floor ${this.stage.floor} · Level ${this.player.level} · ${Math.floor(this.elapsed)}s survived · ${this.save.gold} total gold`);
    }
    draw() {
    ...
    drawMaze(cameraX, cameraY, width, height) {
        const ctx = this.context;
    ...
    drawTraps() {
        ...
        for (const point of this.maze.traps) {
            ...
            if (!this.maze.cells[point.y][point.x].discovered && !this.visitedTraps.has(key))
                continue;
            const center = this.cellCenter(point);
            ctx.save();
    ...
    drawLighting() {
        const ctx = this.context;
        const gradient = ctx.createRadialGradient(this.canvas.width / 2, this.canvas.height / 2, 120, this.canvas.width / 2, this.canvas.height / 2, Math.max(this.canvas.width, this.canvas.height) * 0.7);
    ...
    revealAroundPlayer() {
        ...
        for (let y = cell.y - 3; y <= cell.y + 3; y += 1) {
            for (let x = cell.x - 3; x <= cell.x + 3; x += 1) {
                if (this.maze.cells[y]?.[x] && Math.hypot(x - cell.x, y - cell.y) <= 3.4)
                    this.maze.cells[y][x].discovered = true;
            }
        }
    }
    updateFlowField() {
        ...
        while (cursor < queue.length) {
            ...
            for (const offset of [{ x: 1, y: 0 }, { x: -1, y: 0 }, { x: 0, y: 1 }, { x: 0, y: -1 }]) {
                ...
                if (!this.maze.cells[y]?.[x] || this.maze.cells[y][x].wall || this.distanceMap[y][x] <= nextDistance)
                    continue;
    ...
    loadSave() {
        try {
            ...
            return { ...structuredClone(defaultSave), ...JSON.parse(stored) };
        }
        catch {
            return structuredClone(defaultSave);
        }
        ...
        window.addEventListener("keydown", (event) => {
            ...
            if (event.code === "Space") {
                ...
                if (this.running && !this.paused)
                    this.dash(this.lastInputX, this.lastInputY);
            }
            if (event.code === "KeyE" && this.nearestChest && !this.paused)
                this.openChest(this.nearestChest);
        });
