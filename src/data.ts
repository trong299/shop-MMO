...
export function getStageConfig(floor: number): StageConfig {
  ...
  return {
    ...
    mazeWidth: mazeSize,
    mazeHeight: mazeSize,
    duration: 75 + Math.min(45, n * 2),
    enemyHp: 25 * Math.pow(1.17, n - 1),
    enemyDamage: 7 * Math.pow(1.12, n - 1),
    enemyCount: 5 + Math.floor(n * 0.75),
    eliteChance: 0.03 + n * 0.007,
    trapCount: 3 + Math.floor(n * 0.7),
    treasureCount: 4 + Math.floor(n / 4),
